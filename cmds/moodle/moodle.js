...
const filemanager = require('./CoursesFileManager')
module.exports =
    class Moodle {
      constructor (args) {
      ...
      async _end () {
        if (this.courses) {
          filemanager.update(this.courses)
      ...
      async _add_hvps_on_sections (section, section_id, course_id, Lesson_level, course_short_name) {
        for (let j = 0; j < section.activities.length; j++) {
          ...
          if (!activity.id) {
            ...
            this.courses[course_short_name].sections[section_id].activities[j] = activity
            filemanager.update(this.courses)
          }
          ...
          if (!fs.existsSync(file)) {
            ...
            if (this.config.moodle.theme === 'snap') {
              activity_string = await page.evaluate(
                (section_id) => {
                  if (window.location.href.includes('hvp')) {
                    var url = new URL(window.location.href)
                    return url.searchParams.get('id')
                  } else {
                    let lies = document.querySelectorAll('#section-' + section_id + ' .content ul.section>li.activity')
                    let activity_string = lies[lies.length - 1].id
                    return activity_string.split('-')[1]
                  }
                }, section_id
              )
            } else {
              activity_string = await page.evaluate(
                (section_id) => {
                  let activity_string = document.querySelector('#section-' + section_id + ' .content ul.section>li:nth-last-child(1)').id
                  return activity_string.split('-')[1]
                }, section_id
              )
            }

            let activity_id = activity_string
